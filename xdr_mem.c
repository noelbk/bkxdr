...
static bool_t xdrmem_putbytes (XDR *, char *, u_int);
static u_int xdrmem_getpos (XDR *);
static bool_t xdrmem_setpos (XDR *, u_int);
static int32_t *xdrmem_inline (XDR *, u_int);
static void xdrmem_destroy (XDR *);
static bool_t xdrmem_getint32 (XDR *, int32_t *);
static bool_t xdrmem_putint32 (XDR *, int32_t *);
...
xdrmem_create (XDR *xdrs, caddr_t addr, u_int size, enum xdr_op op)
{
    xdr_init(xdrs);
  xdrs->x_op = op;
  /* We have to add the since the `struct xdr_ops' in `struct XDR'
     is not `'.  */
 ...
 * point at the next element.  Neither object pointed to is 
 */
static bool_t
 ...
 * a good idea.  None of the things pointed to are .
 */
static bool_t
 ...
 * unaligned data.  The source address is .
 */
static bool_t
