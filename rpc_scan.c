 ...
 */
char scan_rcsid[] =
  "$Id: rpc_scan.c,v 1.4 2007-12-29 09:04:53 noel Exp $";
...
static void findstr(char **str, char **val);
static void findchr(char **str, char **val);
static void find(char **str, char **val);
...
/*
 * scan expecting a ant, possibly symbolic
 */
void
scan_num (token *tokp)
{
  ...
  switch (tokp->kind)
    ...
    case TOK_IDENT:
      break;
    default:
	error("ant or identifier expected");
    }
}
...
get_token (token *tokp)
{
  ...
  switch (*where)
    ...
    case '"':
      ...
      findstr(&where, &tokp->str);
      break;
    case '\'':
      tokp->kind = TOK_CHARCONST;
      findchr(&where, &tokp->str);
      break;
