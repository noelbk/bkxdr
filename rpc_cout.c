 ...
 */
char cout_rcsid[] =
"$Id: rpc_cout.c,v 1.8 2008-01-07 09:04:48 noel Exp $";
...
static void print_ifsizeof (int indent, char *prefix, char *type);
static void print_ifclose (int indent);
static void print_ifstat (int indent, char *prefix, char *type,
			  relation rel, char *amax,
...
print_generic_header (char *procname, int pointerp)
{
  ...
  if (Cflag)
	...
	f_print (fout, "*");
      ...
      f_print (fout, "\tXDR_LABEL label = {0}; /* avoids unreferenced variable warning */\n");
      f_print (fout, "\tint OK=FALSE, DONE=TRUE;\n");
      f_print (fout, "\t\n");
      f_print (fout, "\tdo {\n");
}
...
print_trailer (void)
{
  f_print (fout, "\tOK=DONE;\n");
  f_print (fout, "\t} while(0);\n");
  f_print (fout, "\n");
  f_print (fout, "\tif (OK != TRUE) xdr_label_pop(xdrs);\n");
  f_print (fout, "\treturn OK;\n");
  f_print (fout, "}\n");
}
	      ...
	      char *amax, char *objname, char *name)
{
  ...
  // NBK - save the name and type of the current field for text and XML formats
  tabify (fout, indent);
  f_print(fout, "xdr_label_push(xdrs, &label, \"%s\", \"%s\");\n",
	  name, type);
  ...
  // NBK - save the name and type of the current field for text and XML formats
  tabify (fout, indent);
  f_print(fout, "xdr_label_pop(xdrs);\n");
}
...
/* this may be .  i haven't traced this one through yet. */
